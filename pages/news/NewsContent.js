const newsContent = [
  {
    image: 'enugu_provides_land_to_family_homes_funds_for_housing_scheme.jpeg',
    title: 'NSHP: Enugu Provides Land To Family Homes Funds for Housing Scheme',
    slug: 'enugu-provides-land-to-family-homes-funds-for-housing-scheme',
    date: '2nd Mar, 2021.',
    desc:
      'As the National Social Housing Programme (NSHP) that plans to build 300,000 houses affordable houses for low-income earners across the country continues to gain momentum, more states are signing up for the programme. The latest is Enugu state which has provided up to 60 hectares...',
    content:
      '<p>As the National Social Housing Programme (NSHP) that plans to build 300,000 houses affordable houses for low-income earners across the country continues to gain momentum, more states are signing up for the programme.</p>' +
      '<p>The latest is Enugu state which has provided up to 60 hectares of land in Agbamere Eha-Alumona for the programme.</p>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>The programme which is implemented by Family Homes Funds will cater mostly to Nigerians on low and middle income and create jobs which will help stimulate the economy and sustain millions of households.</p>' +
      '<img src="enugu_provides_land_to_family_homes_funds_for_housing_scheme_2.jpeg" alt="Enugu" />' +
      '<p>Family Homes Funds has already developed One 2 Habitable Room Units and 3 Habitable Room Units which are priced from N2million and above for the NSHP in Dei Dei, Abuja. This was done to test the feasibility of the NSHP price points and acceptability of the housing among potential offtakers. After the launch of the model houses, the site was visited by the Vice President on the 2nd of January 2021.</p>' +
      '<p>Family Homes Funds has engaged with all 36 states and the FCT to provide land with adequate infrastructure for the National Social Housing Programme (NSHP).</p>' +
      '<p>The Fund also recently agreed on a partnership with Solar Home System Providers to provide off-grid power systems to all NSHP homes at very affordable rates for the low-income beneficiaries.</p>' +
      '<p>As at now, over 1,500 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi, Osun, Kwara, Enugu.</p>' +
      '<p>In order to ease the process, a portal www.nshp.gov.ng has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'ebonyi_joins_nshp_provides_land_to_family_homes_funds_for_housing_scheme.jpeg',
    title:
      'Ebonyi Joins NSHP, Provides Land to Family Homes Funds for Housing Scheme',
    slug:
      'ebonyi-joins-nshp-provides-land-to-family-homes-funds-for-housing-scheme',
    date: '24th Feb, 2021.',
    desc:
      'Ebonyi state has become the latest state to show interest in the National Social Housing Programme (NSHP) that plans to build 300,000 affordable houses for low-income earners across the country...',
    content:
      '<img src="ebonyi_joins_nshp_provides_land_to_family_homes_funds_for_housing_scheme_2.jpeg" alt="Enugu" />' +
      '<p>Ebonyi state has become the latest state to show interest in the National Social Housing Programme (NSHP) that plans to build 300,000 affordable houses for low-income earners across the country.</p>' +
      '<p>At least 10 hectares have been provided by the state in Umuobuna, Uburu, Ohaozara local government area and zone 12, centenary city, Abakaliki for this development.</p>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>The programme which is implemented by Family Homes Funds will cater mostly to Nigerians on low and middle income and create jobs which will help stimulate the economy and sustain millions of households.</p>' +
      '<p>Family Homes Funds has already developed One 2 Habitable Room Units and 3 Habitable Room Units which are priced from N2million and above for the NSHP in Dei Dei, Abuja. This was done to test the feasibility of the NSHP price points and acceptability of the housing among potential offtakers. After the launch of the model houses, the site was visited by the Vice President on the 2nd of January 2021.</p>' +
      '<p>Family Homes Funds has engaged with all 36 states and the FCT to provide land with adequate infrastructure for the National Social Housing Programme (NSHP).</p>' +
      '<p>The Fund also recently agreed on a partnership with Solar Home System Providers to provide off-grid power systems to all NSHP homes at very affordable rates for the low-income beneficiaries.</p>' +
      '<p>As at now, over 1,500 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi, Osun, Kwara. Construction work starts in up to 12 sites from January</p>' +
      '<p>In order to ease the process, a portal www.nshp.gov.ng has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'osun_kwara_latest_to_provide_lands_to_family_homes_funds_for_low_cost_housing_projects.png',
    title:
      'NSHP: Osun, Kwara, Latest to Provide Lands to Family Homes Fund for Low Cost Housing Projects',
    slug:
      'osun-kwara-latest-to-provide-lands-to-family-homes-funds-for-low-cost-housing-projects',
    date: '27th Jan, 2021.',
    desc:
      'In order to key into the National Social Housing Programme (NSHP), Osun and Kwara states are the latest to provide land title documents to Family Homes Funds for the development of...',
    content:
      '<p>In order to key into the National Social Housing Programme (NSHP), Osun and Kwara states are the latest to provide land title documents to Family Homes Funds for the development of affordable houses for low-income earners in the states.</p>' +
      '<p>In Osun, 61 hectares have been provided for this development in Ilesha, Osogbo, and Ile-ife, while 100 hectares have been provided by Kwara at a site in Illorin, the state capital.</p>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>The programme which is implemented by Family Homes Funds will cater mostly to Nigerians on low and middle income and create jobs which will help stimulate the economy and sustain millions of households.</p>' +
      '<img src="osun_kwara_latest_to_provide_lands_to_family_homes_funds_for_low_cost_housing_projects_2.jpeg" alt="NSHP: Osun, Kwara" />' +
      '<p>Family Homes Funds has already developed One 2 Habitable Room Units and 3 Habitable Room Units which are priced from N2million and above for the NSHP in Dei Dei, Abuja. This was done to test the feasibility of the NSHP price points and acceptability of the housing among potential offtakers. After the launch of the model houses, the site was visited by the Vice President on the 2nd of January 2021.</p>' +
      '<p>Family Homes Funds has engaged with all 36 states and the FCT to provide land with adequate infrastructure for the National Social Housing Programme (NSHP).</p>' +
      '<p>The Fund also recently agreed on a partnership with Solar Home System Providers to provide off-grid power systems to all NSHP homes at very affordable rates for the low-income beneficiaries.</p>' +
      '<p>As at now, over 1,400 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi, Osun, Kwara. Construction work starts in up to 12 sites from January</p>' +
      '<p>In order to ease the process, a portal www.nshp.gov.ng has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'family_homes_funds_partner_off_grid_developers_to_power_nshp_houses.jpeg',
    title:
      'Family Homes Funds Partner Off-Grid Developers to Power NSHP Houses',
    slug: 'family_homes_funds_partner_off_grid_developers_to_power_nshp_houses',
    date: '27th Jan, 2021.',
    desc:
      'Family Homes Funds Limited has reached an agreement with Solar Home System Providers to provide off-grid solar power to all houses under the National Social Housing Programme (NSHP)...',
    content:
      '<p>Family Homes Funds Limited has reached an agreement with Solar Home System Providers to provide off-grid solar power to all houses under the National Social Housing Programme (NSHP).</p>' +
      '<p>The off-grid power arrangement which will provide affordable access to power for the low-income beneficiaries is also one of the components of the Federal Government’s Economic Sustainability Plan (ESP) – a comprehensive strategy to address the health and economic challenges of the Coronavirus in Nigeria.</p>' +
      '<div class="img-text"><img src="family_homes_funds_partner_off_grid_developers_to_power_nshp_houses_2.jpeg" alt="Grid" /><p>The agreement with off-grid power solution ensures that Family Homes Funds houses under the NSHP have access to off-grid solar power. This, according to the MD of Family Homes Funds, Femi Adewole is a ‘’joint off-thinking’’ partnership with the Power Sector Economic Sustainability Committee that will give people on low-income access to shelter and power at an affordable price.</p></div>' +
      '<p>‘’Given the ever-increasing cost of power, this will be a great relief for the beneficiaries, and for their businesses too. Access to affordable and constant power will create economic development. It will help the households set up small and medium businesses to support themselves. This development adds great value to the NSHP and for the intended beneficiaries.’’</p>' +
      '<p>At take-off, the Solar Home System Providers that will be installing the off-grid solar systems at Family Homes Funds sites are Central Electric and Utilities Limited, Sandstream, Emel Solar Solutions and Lumos Nigeria.</p>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is being implemented by Family Homes Funds and is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>As at now, over 1,400 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi, Osun, Kwara. Construction work starts in up to 12 sites from January.</p>' +
      '<p>In order to ease the process, a portal www.nshp.gov.ng has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'family_homes_funds_partners_dangote_cement_for_delivery_of_300000_nshp_houses.jpeg',
    title:
      'Family Homes Funds Partner Dangote Cement for Delivery of 300,000 NSHP Houses',
    slug:
      'family-homes-funds-partners-dangote-cement-for-delivery-of-300000-nshp-houses',
    date: '20th Jan, 2021.',
    desc:
      'With the task to implement the Nigeria government’s National Social Housing Programme (NSHP), Family Homes Funds has struck partnership with Africa’s largest cement producers, Dangote Cement to deliver 300,000 affordable...',
    content:
      '<div class="img-text"><img src="family_homes_funds_partners_dangote_cement_for_delivery_of_300000_nshp_houses_2.jpeg" alt="Grid" /><p>With the task to implement the Nigeria government’s National Social Housing Programme (NSHP), Family Homes Funds has struck partnership with Africa’s largest cement producers, Dangote Cement to deliver 300,000 affordable houses in the Federal Capital Territory and 36 states of the country. <br />Following a series of management meetings between both parties, Dangote Cement has now committed to supplying cement across Family Homes Funds sites at heavily discounted prices. <br /> <br />Given that the objective of the NSHP is to deliver exceptionally affordable homes for Nigerians on low income, this development guarantees a further reduction in the cost of construction of the 300,000 houses.</p></div>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>As at now, over 1,250 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi. Construction work will start in up to 12 sites in January.</p>' +
      '<p>In order to ease the process, a portal <a href="https://www.nshp.gov.ng">www.nshp.gov.ng</a> has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'jigawa_others_provide_lands_to_family_homes_funds_for_low_cost_housing_projects.jpeg',
    title:
      'Jigawa, Others Provide Lands To Family Homes Funds for Low Cost Housing Projects',
    slug:
      'jigawa-others-provide-lands-to-family-homes-funds-for-low-cost-housing-projects',
    date: '19th Jan, 2021.',
    desc:
      'In a bid to key into the National Social Housing Programme (NSHP), the Jigawa state government has provided land title documents to Family Homes Funds for the development of up to 8,000 housing units across four emirates in the state...',
    content:
      '<p>In a bid to key into the National Social Housing Programme (NSHP), the Jigawa state government has provided land title documents to Family Homes Funds for the development of up to 8,000 housing units across four emirates in the state.</p>' +
      '<p>The four emirates are Dutse, Ringin, Kazauri and Hadejia.</p>' +
      '<p>The National Social Housing Programme (NSHP) which is Nigeria government’s response to the health and economic challenges of Coronavirus is going to provide 300,000 homes and 1,800,000 jobs across the 36 states and the Federal Capital Territory of Nigeria.</p>' +
      '<p>The programme which is implemented by Family Homes Funds will cater mostly to Nigerians on low and middle income and create jobs which will help stimulate the economy and sustain millions of households.</p>' +
      '<p>Family Homes Funds has already developed One 2 Habitable Room Units and 3 Habitable Room Units which are priced from N2million and above for the NSHP in Dei Dei, Abuja. This was done to test the feasibility of the NSHP price points and acceptability of the housing among potential offtakers. After the launch of the model houses, the site was visited by the Vice President on the 2nd of January 2021.</p>' +
      '<img src="jigawa_others_provide_lands_to_family_homes_funds_for_low_cost_housing_projects_2.jpeg" alt="NSHP: Jigawa" />' +
      '<p>Family Homes Funds has engaged with all 36 states and the FCT to provide land with adequate infrastructure for the National Social Housing Programme (NSHP).</p>' +
      '<p>Given the positive responses from states so far, over 1,250 hectares of land has been committed for the NSHP in the FCT, Kwara, Plateau, Nasarawa, Ebonyi, Enugu, Anambra, Abia, Delta, Ogun, Osun, Ekiti, Jigawa, Kebbi, Sokoto, Kaduna, Katsina, Yobe, Borno, Bauchi. Construction work will start in up to 12 sites in January</p>' +
      '<p>In order to ease the process, a portal www.nshp.gov.ng has been created to enable citizens’ expression of interest in the programme and to provide stakeholders access to information and opportunities.</p>',
  },
  {
    image:
      'vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja.jpeg',
    image2:
      'vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_2.jpeg',
    image3:
      'vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_3.jpeg',
    image4:
      'vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_4.jpeg',
    title:
      'Vice President Yemi Osinbajo Visits Completed NSHP Model Houses In Dei Dei, Abuja',
    slug:
      'vice-president-yemi-osinbajo-visits-completed-nshp-model-houses-in-dei-dei-abuja',
    date: '3rd Jan, 2021.',
    desc:
      'The Vice President, who was accompanied by Sani Yusuf, the Chief Housing Investment Officer (CHIO) of Family Homes Funds expressed his satisfaction with the quality of the one-bedroom and two-bedroom model houses for low and middle income earners in Nigeria...',
    content:
      '<img src="vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_2.jpeg" alt="" />' +
      '<p>The Vice President, who was accompanied by Sani Yusuf, the Chief Housing Investment Officer (CHIO) of Family Homes Funds expressed his satisfaction with the quality of the one-bedroom and two-bedroom model houses for low and middle income earners in Nigeria.</p>' +
      '<img src="vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_3.jpeg" alt="" />' +
      '<img src="vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_4.jpeg" alt="" />' +
      '<img src="vice_president_yemi_osinbajo_visits_completed_nshp_model_houses_in_dei_dei_abuja_5.jpeg" alt="" />' +
      '<p>The VP said that on the average, anyone earning ₦30,000 a month should be able to spend a third of that amount to acquire these homes that can be as affordable as ₦2,000,000 per unit. He also commended Family Homes Funds for their thoroughness and project delivery.</p>',
  },
  {
    image: 'low_income_earners_set_to_become_happy_homeowners.jpg',
    title: 'Low Income Earners Set to Become Happy Homeowners',
    slug: 'low-income-earners-set-to-become-happy-homeowners',
    date: '5th Oct, 2020.',
    desc:
      'The dream of the average person is to afford a decent and affordable home while being able to meet other basic needs. While this desire rests within most individuals, it’s realization is often short lived as the economic resources required to achieve this aspiration is not within reach...',
    content:
      '<p>The dream of the average person is to afford a decent and affordable home while being able to meet other basic needs. While this desire rests within most individuals, it’s realization is often short lived as the economic resources required to achieve this aspiration is not within reach. This consequently resulted in the emergence of slum settlements as decent housing seems to be outside the reach of many low income earners.</p>' +
      '<p>In sub-saharan Africa where the cost of housing is relatively high, it is often a luxury to own a home despite the basic nature of shelter needs. The situation is more dire for low income earners who have other needs such as food and clothing to consider before making any investments in house ownership.</p>' +
      '<p>Owning a home is indeed a Nigerian dream as most people share this aspiration, hoping it becomes a reality with time. This dream is often short circuited as the demands of the time makes it difficult for them to bear the financial responsibility that comes with owning a home. From land purchase to construction of houses, it’s capital intensive nature makes home ownership a distant prospect.</p>' +
      '<p>Those who can afford to own homes are often at the high end of the spectrum who lease them out at exorbitant costs making it difficult for low income earners to afford. Little wonder why there is proliferation of slum settlements where its dwellers share facilities and live in unsanitary conditions.</p>' +
      '<p>It is indeed a new dawn for Nigerians as a ray of hope has shone its light on low income earners with the launch of the National Social Housing Programme (NSHP). This was initiated to provide decent and affordable homes for low income earners who face housing challenges and economic difficulties. With a disbursement of 200 billion issued by the Central Bank of Nigeria (CBN) on behalf of the government, Nigerians in shared facilities and unsanitary conditions will have the opportunity to own decent low cost houses.</p>' +
      '<p>Starting this year, 300,000 homes will be constructed in all states of the federation including the Federal Capital Territory (FCT). Interestingly, with the <a href="https://fhfl.com.ng/" target="_blank">Family Homes Funds (FHF)</a> as the lead developer, other Nigerian partnerships and collaborations are welcomed to facilitate the construction of these homes. The implication of which is the creation of over 1.8 million jobs for Nigerians across the construction value chain.</p>' +
      '<p>The drive towards providing social housing for Nigerians will ultimately reduce the socioeconomic challenges that emerge from the development of slum settlements. This is in consonance with the <a href="https://sdgs.un.org/goals/goal11" target="_blank">Sustainable Development Goal (SDGs 11)</a> aimed at making cities and human settlements inclusive, safe, resilient and sustainable.</p>' +
      '<p>More than a project, this initiative will help the average low income earner realize their Nigerian dreams. Consequently a more inclusive and resilient society is achieved where the aspirations of its people are validated.</p>',
  },
  {
    image:
      'social_housing_workshop_in_partnership_with_cooperative _federation_of_nigeria.jpg',
    title:
      'Social Housing Workshop In Partnership With Cooperative Federation Of Nigeria',
    slug:
      'social-housing-workshop-in-partnership-with-cooperative-federation-of-nigeria',
    date: '29th Sep, 2020.',
    desc:
      'In a bid to extend its affordable social housing program across Nigeria, Family Homes Fund Limited has revealed that it is already in partnership with cooperatives in Nigeria to provide affordable housing to Nigerians through the...',
    content:
      '<p>In a bid to extend its affordable  social housing program across Nigeria, Family Homes Fund Limited has revealed that it is already in partnership with cooperatives in Nigeria to provide affordable housing to Nigerians through the National social housing programme of the Federal Government.</p>' +
      '<p>The social housing program is about providing homes for people on low income. It is one of the key element of the Economic Sustainability Plan of the Federal Government of Nigeria as one of the key measures to alleviate the consequences of the Covid-19 pandemic which has rendered many low-income earners jobless and homeless.</p>' +
      '<p>The key objective of the social housing program is to create jobs at scale, especially for low income earners whom may have lost their jobs because of the pandemic.</p>' +
      '<p>The second objective is to revitalize the local economy by using local content for housing development. In realising that, about 1,500,000 million doors, 1,800,000 windows, 7,800,000 door hinges and other building materials will be manufactured locally for the social housing program, giving a new lease of life to local industries.</p>' +
      '<p>The third objective is to create affordable and sustainable homes for the low-income earners. Standard house types have been designed for the program to be affordable and super-efficient. On average the prices for a one bedroom will range between ₦1,800,000 to ₦2,000,000 while the two bedrooms will be for about ₦2,700,500.</p>' +
      '<p>Speaking at the workshop organized in conjunction with the Cooperative Federation of Nigeria, the Managing Director of Family Homes Funds, Mr Femi Adewole said “in designing the National Social Housing Program, we are very clear that addressing the demand side is a very critical factor for success. And one of the ways to achieve that was to work with housing cooperatives”,</p>' +
      '<p>He also said ‘’For us, there is no better way to achieve what we want than working with the very many cooperatives that we have in Nigeria. So, our immediate step was to strike a partnership with the cooperative federation of Nigeria; basically, to facilitate access to the over 100,000 cooperatives that we have in Nigeria, and working through them to mobilize demand to feed the program.</p>' +
      '<p>Speaking further, he said part of the essence of the workshop is to finalize on the acceptable criteria for engagement and for the provision of these affordable houses for those who need them most.<br>‘’We already have our own ideas about the criteria for selecting them. Some of those include strong corporate governance. We want cooperatives that are well organized and well run. We want a very even spread across the nation. We also want cooperatives that are socially inclusive and not discriminatory against gender, tribe, religion or age.</p>' +
      '<p>Having said that, The Managing Director reiterated the fact that perhaps one item that is non-negotiable is the issue of affordability and rightful beneficiaries – low income earners.<br>He also alluded to the recent CBN announcement of ₦200billion initial funding for the social housing program. To achieve the intended results, he said the cooperatives must work together with FHF.<br>' +
      '“The fund comes to FHF through the ministry of finance, then we loan it out at a concessionary price to cooperatives to be able to provide houses for their co-operators once they meet the criteria for the loans.<br>' +
      '“The financing will be single digit of 8 to 9% per annum. You will be getting a land subsidy. The land will be provided by public sources like state governments and public services.”<br>' +
      '“When the houses are completed, individual members of cooperatives will buy the houses through a CBN facility, directly or through your cooperative mortgage banks that allow you to pay gradually for the houses.”<br>' +
      '“Cooperatives are significant partners in delivering the social housing program because you can help us to deliver the real needs.”<br>' +
      'He also mentioned that a housing portal will be established where the beneficiaries will access whatever information they need and also provide the information required from them.</p>',
  },
]

export default newsContent
